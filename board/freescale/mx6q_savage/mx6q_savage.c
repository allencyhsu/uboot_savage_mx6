...
static int setup_pmic_voltages(void)
{
	...
	if (!i2c_probe(0x34)) {
		if (i2c_read(0x34, 0, 2, &value, 2))
			printf("%s:i2c_read:error\n", __func__);
		printf("Found WM8326! device id=%x\n", value[0] << 8 | value[1]);
...
{
	/* No delay required on SAVAGE board SD ports */
	return 0;
}
#endif
...
*could not be power off anymore*/
	...
	/* board id for linux */
	gd->bd->bi_arch_number = MACH_TYPE_MX6Q_SAVAGE;
...
int checkboard(void)
{
	printf("Board: %s-SAVAGE: %s Board: 0x%x [",
	mx6_chip_name(),
	mx6_board_rev_name(),
	fsl_system_rev);
